...
var layer_pvci = L.tileLayer('https://public.sig.rennesmetropole.fr/geowebcache/service/tms/1.0.0/ref_fonds%3Apvci_fondplan@EPSG%3A3857@png/{z}/{x}/{y}.png', {
...
$.getJSON(base_url_cadastre + 'communes', function(liste_communes) {
	var data_sites_orgs = $.map(liste_communes, function (commune) {
    	return { text: commune.name, id: commune.idComm }
	});

	afficher_select2('liste_communes', data_sites_orgs, 'choix de la commune', true, false);
...
$.getJSON('assets/json/donnees_sites_orgs.json', function(donnees_sites_orgs) {
	var data_sites_orgs = $.map(donnees_sites_orgs.sites_organismes, function (donnees) {
    	return { text: donnees.text, id: donnees.couche }
	});

	afficher_select2('liste_site_org', data_sites_orgs, 'ajouter un theme base sites & organismes', false, true);
...
$.getJSON('assets/json/donnees_metiers.json', function(donnees_metiers) {
	var data_donnees_metiers = $.map(donnees_metiers.donnees_metiers, function (donnees) {
    	return { text: donnees.nom, id: donnees.couche }
	});

	afficher_select2('liste_equipements_techniques', data_donnees_metiers, 'ajouter donnees metiers', false, true);
...
function generer_url_sviewer() {
	...
	var theme_site_orgs_selectionnes_length = theme_site_orgs_selectionnes.length;
	var equipements_techniques_selectionnes_length = equipements_techniques_selectionnes.length;
	var url_layers = '';
